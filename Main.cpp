...
void ejercicio1(){
	...
	//	vector<string> list;
		...
		for(int i = 0; i < 4 ; i++){
			...
			list.push_back(fechaFinal2[i]);		
		}
		cout << fechaFinal2[0] << ", " << dia << " de " << fechaFinal2[2] << " del " << ano; 
		//cout << ", " << "de " << fechaFinal[]
	...
	switch(opcion){
		case 1:
			cout<<"ingrese el nombre del mes (todo en minuscula): ";
			string mesIngresado2 = "";
			cin >>mesIngresado2;
			for(int i = 0; i < list.size(); i+=4){
				if(list[i+2] == mesIngresado2){
					cout << list[i] << ", " << list[i+1] << " de " <<
					list[i+2] << " del " << list[i+3] << endl;
				}
			}
			break;
/*		case 2:
			...
			break;
		case 3:
			cout << "ingrese el nombre del dia (ejm: Jueves): "<< endl;
			string diaIngresado2 = "";
			cin >> diaIngresado2;
			for(int i = 0; i < list.size(); i+=4){
				if(list[i] == diaIngresado2){
					cout << list[i] << ", " << list[i+1] << " de " <<
					list[i+2] << " del " << list[i+3] << endl;
				}
